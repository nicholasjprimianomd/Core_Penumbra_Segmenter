# -*- coding: utf-8 -*-
# Modified for CPAISD head CT dataset
# %% import packages
...
modality = "CT"
anatomy = "Head"  # anatomy + dataset name
img_name_suffix = "_0000.nii.gz"
gt_name_suffix = ".nii.gz"
prefix = modality + "_" + anatomy + "_"

# Paths to the converted CPAISD dataset in FLARE format
nii_path = "external/MedSAM/data/CPAISD_FLARE_format/images"  # path to the nii images
gt_path = "external/MedSAM/data/CPAISD_FLARE_format/labels"  # path to the ground truth
npy_path = "external/MedSAM/data/npy/" + prefix[:-1]
...
remove_label_ids = []  # Keep all label IDs for the CPAISD dataset
tumor_id = None  # only set this when there are multiple tumors; convert semantic masks to instance masks

# set window level and width for head CT
# https://radiopaedia.org/articles/windowing-ct
WINDOW_LEVEL = 40  # brain/head window level
WINDOW_WIDTH = 80  # brain/head window width narrower than abdominal

# %% save preprocessed images and masks as npz files
# Split into train and validation sets (80/20)
train_names = names[:int(0.8 * len(names))]
val_names = names[int(0.8 * len(names)):]

print(f"Processing {len(train_names)} cases for training")
...
print(f"\nProcessing {len(val_names)} cases for validation")
# Process validation cases the same way as training cases
...
print("\nConversion complete!")
print(f"Processed dataset saved to: {npy_path}")
print(f"- Training cases: {len(train_names)}")
print(f"- Validation cases: {len(val_names)}") 